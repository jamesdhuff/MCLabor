...
BEGIN
	DROP PROCEDURE EMP_LABOR_SUMMARY
END
GO

CREATE PROCEDURE EMP_LABOR_SUMMARY
	@StartDate Date,
	@EndDate Date
AS    
BEGIN
	SELECT	e.employeeId, e.firstName, e.lastName, e.refCode as employeeRefCode,
			ws.workSiteName, ws.refCode as workSiteRefCode, 
			j.jobName, j.refCode as jobRefCode, 		
			CAST((CAST(SUM(DateDiff(minute, dtl.laborStartUTCDateTime, dtl.laborEndUTCDateTime)) AS DECIMAL) / CAST (60 AS DECIMAL)) AS DECIMAL(10,2)) as hoursWorked,
			COUNT(DISTINCT(dtl.laborCalendarDate)) as daysWorked
	FROM	employee e, work_site ws, job j, labor_detail dtl
	WHERE	dtl.employeeId = e.employeeId AND dtl.workSiteId = ws.workSiteId AND dtl.jobId = j.jobId
	AND		dtl.laborEndUTCDateTime IS NOT NULL
	AND		dtl.laborCalendarDate BETWEEN @StartDate AND @EndDate
	GROUP BY e.employeeId, e.firstName, e.lastName, e.refCode,
			ws.workSiteName, ws.refCode,
			j.jobName, j.refCode
END;
